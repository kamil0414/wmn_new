...
export const accruals = await prisma.naliczenie.aggregate({
  _sum: {
    woda: true,
    smieci: true,
  },
...
export const expensesHistory = await prisma.operacja.findMany({
  select: {
    id: true,
    data: true,
    czy_bank: true,
...
export const media = await prisma.opis.findMany({
  select: {
    id: true,
    opis: true,
  },
...
export const plans = await prisma.plan.groupBy({
  by: ["id_opisu"],
  _sum: {
    kwota: true,
  },
