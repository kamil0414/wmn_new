...
export const basicData: any = await prisma.$queryRawUnsafe(
  `select * from PodajDanePomonicze()`,
);

export const blankNumbers = await prisma.numerKP.findMany();

export const flatHistory = await prisma.kartoteka.findMany({
  select: {
    id: true,
    data: true,
    naleznosc: true,
